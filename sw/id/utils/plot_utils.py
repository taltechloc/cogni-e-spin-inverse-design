...
def plot_target_vs_prediction_per_fold(results_dict, method_name, n_folds=5, save_path=None):
    ...
    plt.title(f'{method_name.upper()} Predictions vs Target Diameters per Fold')
...
def plot_target_vs_prediction_overall(results_dict, method_name, save_path=None):
    ...
    plt.title(f"{method_name.upper()} Predictions vs Target")
    plt.tight_layout()
    if save_path:
        plt.savefig(save_path)
    plt.close()
...
def plot_cost_trajectories(fold_cost_histories_all, method_name, save_path=None):
    ...
    plt.title(f'{method_name.upper()} Cost Function Trajectories (Fold-level)')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    if save_path:
